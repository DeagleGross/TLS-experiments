 ...
 * 
 * Architecture:
 *   1. C# accepts connection, gets client socket FD
 *   2. C# calls ssl_connection_create() to create SSL object
 *   3. C# calls ssl_try_handshake() which:
 *      - Calls SSL_do_handshake()
 *      - If complete: returns HANDSHAKE_COMPLETE
 *      - If needs I/O: registers with epoll, returns HANDSHAKE_WANT_READ/WRITE
 *   4. C# calls epoll_wait_one() to wait for socket readiness
 *   5. Repeat steps 3-4 until handshake complete
 ...
 * 
 * In the async-mt C server, each worker thread has its own epoll_fd.
 * Similarly, in C# you might have one epoll per "async context" or share one.
 * 
 ...
 * 
 * CRITICAL: This is what makes SSL_do_handshake() return immediately
 * with SSL_ERROR_WANT_READ/WRITE instead of blocking!
 * 
 ...
 * 
 * This is called after C# accepts a connection. We:
 *   1. Create SSL object from the shared SSL_CTX
 *   2. Associate it with the socket FD (SSL_set_fd)
 *   3. Set it to accept mode (server-side handshake)
 *   4. Make socket non-blocking
 *   5. Register with epoll for EPOLLIN (initial state: waiting for client data)
 * 
 * Like async_mt: we ADD to epoll here, then only MOD in ssl_try_handshake.
 * 
 * Parameters:
...
/**
 * Try to advance the TLS handshake. This is the CORE function.
 * 
 * How it works:
...
int ssl_try_handshake(SSL* ssl, int client_fd, int epoll_fd) {
    // Attempt to progress the handshake
    ...
    // Handshake not complete - check why
    int err = SSL_get_error(ssl, ret);
    
    if (err == SSL_ERROR_WANT_READ || err == SSL_ERROR_WANT_WRITE) {
        // OpenSSL needs I/O - modify epoll registration
        // 
        // WANT_READ means: "I tried to read() but got EAGAIN, no data yet"
        //   → Register for EPOLLIN so epoll wakes us when data arrives
        //
        // WANT_WRITE means: "I tried to write() but got EAGAIN, buffer full"
        //   → Register for EPOLLOUT so epoll wakes us when buffer has space
        ...
        // This matches the async_mt pattern: ADD once on accept, MOD during handshake
 ...
 * Called from C# via Task.Run() to not block the async context.
 * 
 * Parameters:
 ...
 * Useful for epoll registration from C# side.
 */
